...
5. Đăng nhập bot (chọn 1 trong 2 cách)

    **Cách 1 — appstate.json** *(khuyến khích)*
    - Dùng extension [c3c-fbstate](https://github.com/c3cbot/c3c-fbstate) hoặc tool lấy appstate
    - Tạo file `appstate.json` trong thư mục bot, paste nội dung JSON vào, save

    **Cách 2 — cookie.txt** *(chuỗi cookie)*
    - Mở Facebook trên trình duyệt, F12 → Application → Cookies → copy toàn bộ chuỗi cookie
    - Tạo file `cookie.txt` trong thư mục bot, paste chuỗi cookie vào, save

    > ⚠️ Nếu có cả 2 file thì bot sẽ ưu tiên dùng `appstate.json`
